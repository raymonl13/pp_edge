...
if os.getenv("PP_EDGE_TEST_MODE")== "1":
    pytest.skip("skip bankroll pipeline in CI test mode", allow_module_level=True)
...
def test_multi_game_slips():
    assert len(core.run_pipeline(_legs("multi_game.json"), cfg)) >= 1

def test_single_game_zero_slips():
    assert len(core.run_pipeline(_legs("single_game.json"), cfg)) == 0
