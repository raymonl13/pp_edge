...
def build_feature_df(df: pd.DataFrame) -> Tuple[pd.DataFrame, Optional[pd.Series]]:
    ...
    - y is taken from a known target column (case-insensitive) if present; else None.
    """
...
def fit_model(
    ...
    *, random_state: int = 42,
    ...
    """Accept (X,y) or a DataFrame with a target column (case-insensitive); optionally save model and return the Path."""
...
def predict_hit_prob(features: pd.DataFrame) -> np.ndarray:
    """
    Production: require model; unit tests: deterministic fallback.
    """
    if _model is None:
        if _in_test_mode():
            return np.full(len(features.index), 0.5, dtype=float)
