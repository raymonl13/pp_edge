...
exists=False
for j in range(idx[0], idx[upload]):
    if re.search(r'^\s*-\s*name:\s*Build slips\s*$', L[j]): exists=True; break
if not exists:
    ind=re.match(r'^(\s*)', L[idx[upload]]).group(1)
    ins=idx[upload]
    L[ins:ins]=[f"{ind}- name: Build slips", f'{ind}  run: python3 scripts/build_slips.py "$DAY" --cfg config_pp_edge_v6.8.yaml']
wf.write_text("\n".join(L)); print("OK")
