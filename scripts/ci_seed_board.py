...
def make_edges(day):
    ...
    row1={"slip_type":"Power4","legs":str(legs),"edge_pp":-0.375,"stake_total":""}
    row2={"slip_type":"Flex3","legs":str(legs[:3]),"edge_pp":-0.125,"stake_total":""}
    return pd.DataFrame([row1,row2])
def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--day"); ap.add_argument("--out-board",required=True); ap.add_argument("--out-edges",required=True)
    a=ap.parse_args()
