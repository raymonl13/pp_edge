...
def main():
    ...
    if (not csv_path) or (not csv_path.exists()):
        out = {"parity":"ERROR","reason":"EDGE_SHEET_MISSING","missing":[],"extra":[],"type_mismatch":[],"schema_version":None}
        Path("model_parity.json").write_text(json.dumps(out,indent=2))
        write_meta({"MODEL_STATE":"ERROR","MODEL_REASON":"EDGE_SHEET_MISSING"})
    ...
    missing = sorted([f for f in expected.keys() if f not in found_types])
    extra = sorted([c for c in found_types.keys() if c not in expected])
    mismatched = []
    ...
    Path("model_parity.json").write_text(json.dumps(out,indent=2))
    if state=="ERROR":
        reason = "missing=" + str(len(missing)) + " mismatched=" + str(len(mismatched))
