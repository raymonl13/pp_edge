...
a=p.parse_args()
day=a.day or str(datetime.date.today())
try: slips=json.load(open(a.inputs))
except: slips=[]
if not isinstance(slips,list): slips=[]
if not slips:
    dc=int(a.daily_max_risk_pct*a.bankroll); open(a.out,"w").write("[]\n"); print(f"bankroll_decider D={day} n=0 total_risk=0.0 daily_cap={dc} cool_off=false"); sys.exit(0)
...
for r in slips:
    ...
    if ev is None or pw is None or od is None: sk="invalid_input"
        ...
        b=od-1.0; k=(pw*b-(1.0-pw))/b; amt=max(0.0,a.kelly_fraction*k*a.bankroll)
        x=min(amt,a.per_slip_max_pct*a.bankroll); rem=max(a.daily_max_risk_pct*a.bankroll - tot,0.0); x=min(x,rem)
