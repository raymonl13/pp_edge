...
def main():
    ...
    for _cp in (f"calibration/{day}.json", "calibration/latest.json"):

        if os.path.exists(_cp):

            try:

                cal=json.load(open(_cp)); break

            except Exception:

                pass

    df["p_cal"]=apply_calibration(df.get("p_raw", pd.Series([float("nan")]*len(df))), cal)
...
def apply_calibration(p, cal):
    ...
    if m == "isotonic" and "pairs" in cal:
        pts = cal["pairs"]
        xs = np.array([float(x) for x, _ in pts]); ys = np.array([float(y) for _, y in pts])
