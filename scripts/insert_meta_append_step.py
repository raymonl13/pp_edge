...
def block(k): s=idx[k]; e=idx[k+1] if k+1<len(idx) else len(L); return s,e
build=None; upload=None
for k in range(len(idx)):
    n=L[idx[k]].strip()
    if re.search(r'^- name:\s*Build slips\s*$', n): build=k
    if re.search(r'^- name:\s*Upload\s*$', n) and upload is None: upload=k
if build is None or upload is None:
    print("MISSING_BUILD_OR_UPLOAD"); exit(0)
bs,be=block(build)
ins=be
...
already=False
for k in range(len(idx)):
    if re.search(r'^\s*-\s*name:\s*Append slip meta lines\s*$', L[idx[k]].strip()): already=True
if not already:
    L[ins:ins]=step
wf.write_text("\n".join(L)); print("OK")
