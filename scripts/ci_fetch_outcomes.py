        ...
        if "legs" in cols:
            ...
            for row in rdr:
                ...
                for lg in payload:
                    ...
                    player=(lg.get("name") or lg.get("player") or "").strip()
                    stat=(lg.get("stat") or "PTS").strip()
                    line=lg.get("line")
                    pr=lg.get("p_raw") or lg.get("p_hit") or lg.get("prob")
                    rows.append({"player":player,"stat":stat,"line":line,"p_raw":pr})
                    k+=1
                    if k>=limit: return rows
        else:
            pcol=None
...
def write_outcomes(day, rows):
    ...
    with open(out, "w", newline="") as fh:
        ...
        for i,r in enumerate(rows):
            ...
            try:
                ln=float(r.get("line")) if r.get("line") is not None else ""
            except Exception:
                ln=""
