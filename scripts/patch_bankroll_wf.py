...
for i,s in enumerate(steps):
    ...
    rn=str(s.get("run") or "")
    if "Assert joined exists" in nm or "ci_ensure_joined_day.py" in rn:
        idx=i+1; break
if idx is None:
    for i,s in enumerate(steps):
        nm=str(s.get("name") or "")
        if "Inspect rescued day (pre)" in nm:
            idx=i+1; break
if idx is None: idx=len(steps)
derive_run="\n".join([
"set -e",
'PY=${PY:-python3}',
'DAY="${{ env.EFFECTIVE_DAY }}"',
'$PY scripts/derive_slips_from_joined.py --day "$DAY" --joined "outcomes/day=$DAY/joined.csv" --out "slips_manual.json" --odds_default 2.18 || true',
'if [ ! -s slips_manual.json ]; then echo "[]" > slips_manual.json; fi'
])
decide_run="\n".join([
"set -e",
'PY=${PY:-python3}',
...
'python - <<\'PY\'',
'import json,os',
'day=os.environ.get("EFFECTIVE_DAY","")',
...
bundle=[{"name":"Derive slips for bankroll","run":derive_run,"shell":"bash"},
        ...
        {"name":"Upload bankroll artifacts","uses":"actions/upload-artifact@v4","with":{"name":"bankroll_${{ env.EFFECTIVE_DAY }}","path":"bankroll_decision.json\nbankroll_summary.txt"}}]
...
print("patched_after_assert_index",idx)
