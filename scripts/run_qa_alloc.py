...
def _apply_overrides(cfg):
    # ENV override for smoke thresholds
    mr = os.getenv("QA_MIN_ROWS")
    if mr:
        try:
            mr_i = int(mr)
            cfg.setdefault("qa", {})["min_rows"] = mr_i
        except Exception:
            pass
    return cfg
...
def main():
    ...
    day = _resolve_day(args.day, args.tz)
    ...
    try:
        if not edge_csv.exists():
            Path("qa_report.json").write_text(json.dumps({
                ...
                "issues": [{"severity": "FAIL", "msg": "edge_csv_missing"}]
            }, indent=2))
        ...
        cfg = _load_yaml(Path(args.cfg)) or {}
        ...
        Path("qa_report.json").write_text(json.dumps({
            "state": "FAIL",
            "rules_version": QA_RULES_VERSION,
            "issues": [{"severity": "FAIL", "msg": f"exception:{type(e).__name__}:{e}"}]
        }, indent=2))
        Path("qa_report.csv").write_text("severity,msg\nFAIL,exception\n")
