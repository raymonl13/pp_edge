...
for j in range(qa+1,up):
    if re.search(r'^\s*-\s*name:\s*Relax QA on SYNTH\s*$',L[j]): exists=True
if not exists:
    ind=re.match(r'^(\s*)',L[qa]).group(1)
    runind=ind+"  "
    L.insert(ins, runind+"- name: Relax QA on SYNTH")
    L.insert(ins+1, runind+"  if: always()")
    L.insert(ins+2, runind+"  run: python3 scripts/qa_relax_on_synth.py")
wf.write_text("\n".join(L))
print("OK")
