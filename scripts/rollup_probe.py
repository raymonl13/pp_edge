...
out = {"day": None, "realized_rows": 0, "prob_rows": 0, "join_rows": 0, "brier": None, "logloss": None}
paths = sorted(glob.glob("realized/realized_*.csv"))
if not paths:
    print(json.dumps(out)); exit(0)
rp = paths[-1]
...
if len(j):
    import numpy as np
    p = j["p"].clip(1e-9, 1-1e-9).astype(float)
    y = j["outcome"].astype(float)
