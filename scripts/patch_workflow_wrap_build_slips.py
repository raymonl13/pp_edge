...
s,e=blk(build_k)
# replace run: block with a wrapped shell that logs everything
run_i=None
...
f'{ind}    echo \"DAY=${{DAY:-}}\"',
f'{ind}    ls -l || true',
f'{ind}    # show scored sheet head',
f'{ind}    if [ -z \"${{DAY:-}}\" ]; then DAY=\"$(date -u +%F)\"; fi',
f'{ind}    ES=\"edge_sheet_${{DAY}}.csv\"',
f'{ind}    echo \"--- HEAD of $ES ---\"; head -n 6 \"$ES\" 2>/dev/null || echo \"missing $ES\"',
f'{ind}    # run builder with dev mode and faulthandler; capture logs',
...
if run_i is None:
    ...
    # delete old run block and replace
